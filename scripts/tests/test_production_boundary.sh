...
const invalidQuota = parseCodexRateLimits({ rateLimits: { rateLimitsByLimitId: { primary: { usedPercent: -20, remainingPercent: 120, windowDurationMins: 300 } } } });
assert.equal(invalidQuota[0].capability_state, "unavailable");
assert.equal(invalidQuota[0].bucket, "unavailable");
