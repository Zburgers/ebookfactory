const state = { projects: [], selected: null, sections: [], eventCursor: Number(localStorage.getItem("ebook-factory-event-cursor") || 0) };
...
async function selectProject(project) { state.selected = project; state.eventCursor = Number(localStorage.getItem(`ebook-factory-event-cursor:${project.project_id}`) || 0); $("#studio-project-label").textContent = project.title; document.querySelector('[data-view="studio"]').click(); await Promise.all([loadMessages(), loadEvents(), loadSections(), loadReviews(), loadUsage()]); }
...
async function loadTelegramStatus() { const status = $("#telegram-status"); try { const telegram = await api("/telegram/status"); status.textContent = telegram.configured ? `Configured · ${telegram.linked_chat_count} linked chat(s) · next update ${telegram.next_update_id}` : "Not configured. Set the local bot token and allowlisted chat/sender IDs; no credentials are shown here."; } catch (error) { status.textContent = error.message; } }
$("#refresh-telegram").addEventListener("click", () => loadTelegramStatus());
await loadProjects(); await loadProviders(); await loadTelegramStatus(); setInterval(() => state.selected && loadEvents().catch(() => {}), 3000);
